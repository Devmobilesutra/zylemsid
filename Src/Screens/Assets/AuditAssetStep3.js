...
import {StyleSheet, View, Text, ImageBackground, TouchableOpacity, ScrollView, Image, FlatList, TextInput,PermissionsAndroid} from 'react-native';
...
var currentDateTimeFilename=''
      ...
      if(this.state.remark.length>0){
        ...
        var date=year + '-' + month + '-' + date + 'T' + hours + ':' + min + ':' + sec
      ...
      AuditDate=moment().format('DD-MMM-YYYY')
         ...
         db.insertImagesDetails(app_order_id,currentDateTime,currentDateTimeFilename,this.state.fileList[i].url.uri,"N")
       
     } 
       db.inserOrderMasterEntryForAsset(app_order_id,currentDateTime,"1",this.props.shops.shopId,this.state.userLatitude,this.state.userLongitude,"","","","5",this.props.dashboard.userId,"",1,"N",newDate,"","")
       db.insertAssetData(app_order_id,this.props.assets.assetID,this.props.assets.assetQRcode,this.props.assets.Scan_status_flags,"",this.state.remark,this.state.condition,AuditDate).then((data)=>{
        Actions.Info()
       })
...
render() {
    const{assetID,assetQRcode,CustomerID,ScanFlag,assetName,assetsType,modelNo,sizeSqFeet,allocatedToName}=this.props.assets
            ...
            <View>
                ...
                <View style={{marginTop:hp('1')}}>
                    ...
                    <View style={styles.nameTextBoxMainContainer}>
                        ...
                        rippleOpacity={0}
                        onChangeText={(value) => { this.onSelectedCondition(value) }}
                    />
                    </View>
                </View>
...
const mapStateToProps = (state) => {
  ...
  const mapDispatchToProps = dispatch => ({
    assetsInfo: (assetID,assetQRcode,CustomerID,ScanFlag,assetName,assetsType,modelNo,sizeSqFeet,allocatedToName) => { dispatch(ASSET_INFO(assetID,assetQRcode,CustomerID,ScanFlag,assetName,assetsType,modelNo,sizeSqFeet,allocatedToName));    },
    assetsInfoFlag: (Scan_status_flag,Asset_info_flag) => { dispatch(ASSET_INFO_FLAG(Scan_status_flag,Asset_info_flag));    },
    qrcode: (qrString) => { dispatch(QR_CODE(qrString));       }
}
  )
  export default connect(mapStateToProps, mapDispatchToProps)(AuditAssetStep3)
