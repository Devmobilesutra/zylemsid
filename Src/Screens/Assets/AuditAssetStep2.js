...
var assetsType = '', CustomerID = '', ScanFlag = '', assetQRcode = '', sizeSqFeet = '', manufacturedBy = '', modelNo = '', serialNo = '', installationDate = '', allocatedTo = '', assetID = '', assetName = ''

var app_order_id
var Scan_status_flag = '', Asset_info_flag = '', ChechAssetstext = "", default_dist = ''
export class AuditAssetStep2 extends Component {
    constructor(props) {
        super(props);
        this.state = {
            assetsType: '',
    ...
    componentDidMount() {
        ...
        db.checkQrCodeDataInDb(this.props.assets.qrString).then((data) => {
            //         [{"AssetID": "1",
                ...
                db.getCustomerName(CustomerID).then((data) => {
                    if (data.length) {
                        for (var i = 0; i < data.length; i++) {
                            ...
                            this.props.assetsInfo(assetID, assetQRcode, CustomerID, ScanFlag, assetName, assetsType, modelNo, sizeSqFeet, allocatedToNames)
            ...
            <Root>
                <View>
                    <FlatList
                        ...
                        aaaa keyExtractor={(item, index) => index.toString()}
                ...
                >
                    ...
                    <View style={{ flexDirection: 'column', alignItems: 'center' }}>
                        <TouchableOpacity onPress={() => this.confirmClickHandler()}>
                            <View style={{
                                backgroundColor: '#221818', height: hp('8'), width: wp('90'),
                                borderRadius: wp('2'), alignItems: 'center', justifyContent: 'center',
                            }}>
                                <Text style={{
                                    color: '#FFFFFF', fontSize: RFValue(14), fontWeight: 'bold',
                        ...
                        <TouchableOpacity onPress={() => this.cancelClickHandler()}>
                            <View style={{ alignItems: 'center', justifyContent: 'center', marginVertical: hp('3') }}>
                                <Text style={{
                                    color: 'black', fontSize: RFValue(14), fontWeight: 'bold',
                                    fontFamily: 'Proxima Nova',
                                }}>
                                    CANCEL
                        </Text>
...
export default connect(mapStateToProps, mapDispatchToProps)(AuditAssetStep2)
