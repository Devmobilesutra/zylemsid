...
componentWillMount(){
    ...
    //entity_id neded
    db. getItemDiscountFromDb(entity_id,orderIds,this.props.ItemId).then((data) => {
...
convertBTLToCS(){
    if(this.state.selectedRatePer == 'CS'){
    var cs =0, btl =0;
    if(!this.state.unit)
    {
    ...
    cs =parseInt(this.state.box) +parseInt(parseInt(this.state.unit) / parseInt(this.state.bpc));
    console.log("cs : "+cs)
    btl = parseInt(parseInt(this.state.unit) % parseInt(this.state.bpc));

    console.log("btl : "+btl)
    this.state.box = cs;
    this.setState({box : cs});
    this.state.unit = btl;
    this.setState({unit : btl});
    console.log('cs & btl : '+this.state.box +" "+this.state.unit)
  }
...
export default connect(mapStateToProps, mapDispatchToProps)(EditInlineOnCreateNewOrder)
