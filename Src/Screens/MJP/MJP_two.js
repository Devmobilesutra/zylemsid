...
// {"OrderMaster":[{"CollectionType":"6","Remark":"Read KARNATAKA","ToDate":"01-Dec-2020","FromDate":"01-Dec-2020","EntityID":"63","EntityType":"3"},{"CollectionType":"6","Remark":"Added KARNATAKA","ToDate":"01-Dec-2020","FromDate":"01-Dec-2020","EntityID":"68","EntityType":"3"}]}
  ...
  db.getMeetForSync().then((data) => {
    if (data.length > 0) {
      ...
      this.state.JSONObj['"OrderMaster"'] = JSON.stringify(data)
      console.log("json",this.state.JSONObj);
  }
                      var count
    ...
    saveMeet = (Meeting_Id,Shop_Id,Shop_name,PlannedDate,Time,location,Remarks,IsActivityDone,Type_sync,collection_type) => {
     var ID=moment().format('YYYYMMDDHHmm');
     db.MeetDraftDetails().then((data)=>{
      this.setState({ MeetDraftDetails: data })
      if(data.length==0)
      {
        db.InsertMeet(ID,Meeting_Id,Shop_Id,Shop_name,PlannedDate,Time,location,Remarks,IsActivityDone,Type_sync,collection_type);
...
>
  ...
  <TextInput
      ...
      onChangeText={text => this.onChangeEnteredBox(text)}/>
            ...
            <TouchableOpacity
          onPress={() => {
            this.saveMeet(this.state.Meeting_Id,this.props.EntityTypeID,this.state.Shop_final,this.props.PlannedDate,"00:00",this.state.Shop_addFinal,this.state.Remarks,this.props.IsActivityDone,"3",this.state.collection_type);
          }}
          style={{
