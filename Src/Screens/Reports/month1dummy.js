...
export class Month1 extends Component {
    ...
    _renderTarget(brandId) {
        var itemarr=[]
        db.getItemIdArray(brandId).then((data)=>{
       itemarr=data
        })
        ...
        if(month == 0){
         ...
         // 2]    Select BPC,PTR from PItem where ItemId='%@'",iid
    ...
    _renderAchieved(brandId) {

    }

    _renderPerGe(brandId) {
        var target1 = "sum(Target)";
        if (month == 0) {
            db.getTargetData(brandId, target1).then((data) => {
                return (<Text style={{ color: '#362828', fontSize: 12 }}>0000000</Text>)
            })
        }
        else {
            db.getTargetDatabymonth(brandId, target1, month).then((data) => {
                return (<Text style={{ color: '#362828', fontSize: 12 }}>0000000</Text>)
            })
        }
...
export default connect(mapStateToProps, mapDispatchToProps)(Month1)
