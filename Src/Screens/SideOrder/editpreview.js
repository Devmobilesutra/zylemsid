...
var totalAmounts
export class editpreview extends Component {
    constructor(props) {
        super(props);
        this.state = {
            a: '',
...
Actions.drawerMenu()
    ...
    componentDidMount() {
       ...
       db.newsidePreview(this.props.order_id).then((data) => {
        console.log("exact data for prev",data);
        this.setState({BrandListU:data})
        console.log("exact oid",this.props.order_id);
        console.log("exact dataarray for prev",this.state.BrandListU);
    ...
    _componentFocused = () => {
        ...
        ///sonali
        AsyncStorage.getItem(this.props.order_id).then((keyValue) => {
            var a = JSON.parse(keyValue)

//             db.getInsertedsTempOrder(this.props.order_id).then((getdata) => {
               
// console.log("getdata in the orderedit preview",JSON.stringify(getdata),this.props.order_id);
//                 this.setState({ BrandList: getdata })
//                 var amountss = 0
//                 for (var i = 0; i < this.state.BrandList.length; i++) {
//                     amountss += parseInt(this.state.BrandList[i].Amount)
...
//             })
    ...
    returnBrand() {
       // const { navigation } = this.props;
        ...
        return this.state.BrandListU.map((item, i) => {
            ...
            <View style={{ flex: 1 }}>
                ...
                >
                    ...
                    <View style={{ flexDirection: 'row' }}>
                        <TouchableOpacity style={styles.buttonContainer} onPress={(e) => this.newsave(e)}>
...
export default connect(mapStateToProps, mapDispatchToProps)(editpreview)
